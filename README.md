...
To build, run:
```bash
bazel build -s --verbose_failures --config=esp32 //:hello
```

To flash a device:
```bash
bazel build --strategy=standalone -s --verbose_failures --config=esp32 //:gen_partitions
bazel build --strategy=standalone -s --verbose_failures --config=esp32 //:gen_image
